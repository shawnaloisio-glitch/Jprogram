...
Isolated copy, adapted from Jprogram's Analysis/corpus_loader.py
(2026-08-07) - not a live import. Language Coach owns this copy entirely;
it has no runtime dependency on Jprogram's code, only on Jprogram's data
(the canonical JSONL corpus), consistent with the one-program-one-task
boundary. See LANGUAGE_COACH_DESIGN_SPEC.md §6.

Deterministic canonical JSONL reader. Reads the canonical sentence-per-line
JSONL corpus in canonical record order.
...
def load_corpus(corpus_path):
    ...
    Expected input:
        ...
        (the canonical corpus produced by Jprogram's Corpus Builder).
