...
Isolated copy, adapted from Jprogram's Analysis/comparison_analyzer.py
(2026-08-07) - not a live import. See corpus_loader.py for the boundary
note.

**Adapted for surface-form grouping (design spec §8a)** for vocabulary
and expressions - see frequency_analyzer.py for the full rationale.
Chunks already group by their own text (already surface-level, no change).

Cross-source comparison. Produces deterministic evidence datasets
comparing two or more canonical corpora. Measures differences between
corpora only. It does NOT decide which corpus is better, rank materials,
recommend study materials, infer difficulty, or infer learning value.

INDEPENDENCE (kept from original design):
...
def analyze(sources):
    ...
    Grouping keys:
        - vocabulary: word.surface (not lemma - see module docstring)
        - expressions: expression.surface (not pattern)
        - chunks: chunk.text (already surface-level)
        ...
        Structured dict suitable for output_writer:
            {
                "comparison": {
                    ...
                    "vocabulary": {
                        ...
                        "by_item": { <surface>: {source: {
                            "occurrences": int}} } },
                    "expressions": {
                        "shared": [...], "unique": {source: [...]},
                        "by_item": { <surface>: {source: int}} },
                    "chunks": {
                        "shared": [...], "unique": {source: [...]},
    ...
    for source in source_names:
        ...
        for record in sources[source]:
            ...
            for word in words:
                ...
                per_source = entry.setdefault(source, {"occurrences": 0})
    ...
    for key in sorted(vocab):
        vocab_by_item[key] = {
            source: {"occurrences": vocab[key][source]["occurrences"]}
