...
Isolated copy, adapted from Jprogram's Analysis/chunk_analyzer.py
(2026-08-07) - not a live import. See corpus_loader.py for the boundary
note. No grouping-key change needed here - chunks already group by their
own canonical text (already surface-level), unlike words/expressions.

Chunk / grammar-pattern analysis. For each chunk (grouped by its
canonical text):
- occurrences
...
recommendations. Grammar/expression known-state tracking itself is
explicitly deferred (see WORKING_LIST.md) - this module is copied now
for completeness, not because grammar work is starting.
...
def analyze(records):
    ...
    Grouping key:
        chunk.text (the canonical chunk text - already surface-level).
