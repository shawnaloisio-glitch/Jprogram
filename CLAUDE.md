...
- **Scope `--allowed-tools` narrowly to exactly what the task needs** (e.g. `Read,Edit,Bash(python*)`) as a matter of practice — but see the known issue below: as of 2026-08-13, `--allowed-tools` alone does not reliably grant reasonix-cli write access in headless invocation. The "Confirmed 2026-08-08" claim this line used to make was carried over unverified from the retired `claude -p` mechanism and was never actually re-tested against reasonix-cli until 2026-08-13, when it turned out not to hold — see below.

**⚠️ Known issue (found 2026-08-13, unresolved): reasonix-cli's non-interactive invocation can hard-block all writes regardless of `--permission-mode`.** First real write task attempted through reasonix-cli (fixing `parser_normalizer.py`'s empty-block bug) failed 5 times in a row with `constraint=no-mutation`, across every combination tried: `-p` with default `ask`, `-p` with `--permission-mode acceptEdits`, `-p` with `--permission-mode bypassPermissions`, and `run` (not `-p`) with `--permission-mode bypassPermissions` — both with and without a copy of the main repo's `reasonix.toml` (and its 64 accumulated allow-rules) placed in the target worktree. `reasonix doctor` confirmed `sandbox.write_roots` correctly followed `--dir` each time, so it isn't a sandbox-path problem; the block is a separate, undiagnosed constraint layer not controlled by `--permission-mode` or `--allowed-tools`. Coder's *reasoning* was not the problem — across all 5 attempts it independently re-derived the identical correct root cause and patch from cold, read-only analysis. Untried as of this writing: the plain interactive `reasonix` session form (no `-p`/`run`), and `reasonix setup`'s config wizard for a possibly-missing one-time step. **Until this is root-caused, do not assume a Coder task will actually be able to write files** — budget for the realistic possibility that Advisor will need to apply Coder's own converged-on, read-only-derived patch directly, as an Owner-authorized one-time exception per task (not a standing default), the way fix #1 above was resolved.
