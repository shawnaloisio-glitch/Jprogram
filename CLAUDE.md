...
- **Scope `--allowed-tools` narrowly to exactly what the task needs** (e.g. `Read,Edit,Bash(python*)`) — this governs which tools the session may attempt, but it is NOT what grants an individual write call permission to execute. See the next point.
- **`reasonix.toml`'s `[permissions] allow` list must contain an explicit `Edit`/`Write` entry, or headless writes hard-fail (RESOLVED 2026-08-13, was a real multi-hour outage).** Root cause: `reasonix`'s permission rules engine (per `~\AppData\Roaming\reasonix\config.toml`'s `[permissions]` docs) checks each write call against `deny > ask > allow > fallback`; with no matching rule the global fallback (`mode = "ask"`) applies, and headless invocation has no way to answer an "ask" prompt, so it hard-denies (`constraint=no-mutation`) — completely independent of `--permission-mode` or `-p` vs `run` (5 attempts across 4 permission-mode/subcommand combinations all failed identically before this was found). This project's `reasonix.toml` had 64 accumulated `Bash=<exact command>` allow-rules but zero `Edit`/`Write` rules, because until 2026-08-13 no headless task had ever attempted a real write (see `DONE.md`/`Audits` — all prior "1-2 days of coding" via reasonix ran through the **desktop app**, source:desktop in `~\AppData\Roaming\reasonix\stats\*.jsonl`, a separate permission path). **Fix, confirmed working against both an isolated scratch dir and this real repo:** add `"Edit", "Write"` to `reasonix.toml`'s `allow` array. `reasonix.toml` is not tracked in git (host-local), so this fix does not travel with the repo — if Coder tasks start failing with `constraint=no-mutation` on a different machine or after a fresh `reasonix.toml`, this is the first thing to check.
- **Self-report unreliability, `constraint=no-mutation`-adjacent:** even after the permission fix, a successful write can still return `is_error: true` / exit code 1 / `subtype: "error_during_execution"` in the JSON result. Always verify the actual file on disk — never treat the JSON `result` text or exit code as sufficient evidence, in either direction (matches the project's existing evidence-hierarchy principle, just doubly load-bearing here).
...
- The task prompt (everything after the stable header) uses the fixed opening template below, verbatim, every time — only the part-count number varies:
  ...
  ```
  You are Coder for the Jprogram Japanese corpus pipeline. You implement; Advisor evaluates your work and reports to Owner, who decides. Read AGENTS.md in the project root now for your full standing operating rules before starting. Execute only the task below, precisely and within its stated boundary — do not modify files outside this list even if you notice something else that looks wrong (report it instead). This task has N enumerated parts — your report must state the status of each part individually (done/not done/blocked), never report only the completed parts as if they were the whole task. End with STOPPED. only when every part is actually done; otherwise ask "Continue to next section?" — never leave a part silently undone.
  ```
...
**Qwen Code is not part of this project's audit model (Owner decision, 2026-08-05 — settled, not under review, do not propose revisiting unprompted).** There is no cross-vendor auditor available, for any tier. The real separation of duties in this setup is: **OC+DeepSeek implements, Claude Code reviews** — a different model doing the implementation is the independence that exists here; a different vendor auditing the review is not. Auditor handles every trigger case — there is no routing split by tier.
...
**Judgment call, every time:** there is no automatic-Yes designation for any file — assess trigger risk case by case based on correctness stakes and blast radius (see "Frozen Components (retired 2026-08-13)" below for what this replaces).
...
## Frozen Components (retired 2026-08-13)

**Removed at Owner's explicit override ("I am overriding you — remove all freezes everywhere"), given plainly by Advisor beforehand: the freeze existed specifically to force independent audit on Parser/Validator/Builder/Transport changes, and it was being removed at the exact moment a live correctness bug was open in two of those files (`parser_normalizer.py`, `deterministic_parser.py` — GiNZA surface-mismatch and off-by-one sentence-count reconstruction failures, see `DONE.md` Session 15 / `TODO.md` Open items). Full record of the exchange and the risk as stated: this session's transcript.**

No file in this project carries an automatic-Yes audit-trigger designation anymore. The "Judgment call, every time" rule above governs all Audit-trigger assessments now, including for the Parser/Validator/Builder/Transport files formerly listed here (`Prompts/parser_prompt.md`, `PARSER_OUTPUT_SPEC.md`, `Data Processor/deterministic_parser.py`, `Data Processor/response_validator.py`, `Data Processor/corpus_builder.py`, `Data Processor/parser_normalizer.py`, `Data Processor/deepseek_client.py`, `Data Processor/deterministic_parser_client.py`).
