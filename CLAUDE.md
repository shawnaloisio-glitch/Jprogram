...
- **Scope `--allowed-tools` narrowly to exactly what the task needs** (e.g. `Read,Edit,Bash(python*)`) — this governs which tools the session may attempt, not whether an individual write call is permitted to execute (see next point).
- **If a headless task hard-fails every write with `constraint=no-mutation`, this project's `reasonix.toml` is almost certainly missing an `Edit`/`Write` entry in `[permissions] allow`.** Root-caused and fixed here 2026-08-13 (was a real multi-hour outage) — full detail, evidence, and the fix in `Shared\RX_WORKFLOW.md`'s "Headless write block" section, not duplicated here. Already applied to this project's `reasonix.toml`; note that file is host-local (not tracked in git), so a fresh checkout on another machine needs the fix reapplied.
