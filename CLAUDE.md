...
**Capture OC's output from `opencode session export <sessionID>` or the raw storage at `~/.local/share/opencode/storage/message/{sessionID}/`, not from terminal display text** — the terminal view is a formatted subset, not the complete record.
