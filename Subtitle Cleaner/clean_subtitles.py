...
Japanese Corpus Pipeline - Subtitle Cleaner (mechanical worker)
...
Consumes exactly one Cleaning Job artifact and produces:
    ...
    - Cleaner log (Logs\\Subtitle Cleaner\\<source_id>.cleaner.log)
...
PROGRAM_VERSION = CLEANER_VERSIONS.get("subtitle_standard_v1", "1.0")

SUBTITLE_PROFILE = "subtitle_standard_v1"
...
def assignment_errors(job):
    ...
    assigned to it: the subtitle source type, the subtitle cleaning
    ...
    elif processing["cleaner"] != "clean_subtitles":
        errors.append(
            f"source_type {source_type!r} is not assigned to the "
            f"subtitle cleaner"
        )

    if job.get("cleaning_profile") != SUBTITLE_PROFILE:
        errors.append(
            f"cleaning_profile {job.get('cleaning_profile')!r} is not "
            f"{SUBTITLE_PROFILE!r}"
        )

    configured = CLEANER_VERSIONS.get(SUBTITLE_PROFILE)
...
def clean_subtitle_text(text):
    """
    Apply the subtitle_standard_v1 transformation sequence.

    Exact order:
        1. Strip the UTF-8 BOM.
        2. Split into lines.
        3. Trim lines.
        4. Remove subtitle number lines (^\\d+$).
        5. Remove SRT timecode lines.
        6. Collapse blank lines.
        7. Join output.

    Input: raw subtitle text (str).
    ...
    kept_lines = []
    subtitle_numbers_removed = 0
    for line in lines:
        if NUMBER_RE.match(line):
            subtitle_numbers_removed += 1
            continue
    ...
    lines, blank_lines_removed = collapse_blank_lines(kept_lines2)

    output_text = join_clean_lines(lines)
    ...
    statistics = {
        ...
        "subtitle_numbers_removed": subtitle_numbers_removed,
        "timecodes_removed": timecodes_removed,
...
def run(job_path):
    ...
    output_text, statistics = clean_subtitle_text(text)
