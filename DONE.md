...
1. **Standalone-import `source_id` collisions remain unfixed** (see `TODO.md`) — real, confirmed with both a clean sha256-based catch and a genuinely silent race-condition case across parallel workers. No fix designed yet.
...
3. **Two Audit-trigger-Yes parser fixes (`d62eeec`, `f400d2d`) still have no independent Auditor pass** — unchanged, carried since Session 16. (The new では fix is a third change in this category, also without an independent Auditor pass — though unlike the other two, it has full corpus-wide ground-truth re-verification behind it.)
...
None outstanding — corpus is at 3,156 sources across 5 creators, the では chunk bug is fully fixed and corpus-wide verified, all commits pushed. Next session picks up wherever Owner wants (the standalone-import collision-bug fix and the Reasonix write-block — now strongly narrowed toward an internal `reasonix-cli` bug — are the two standing open investigations).
